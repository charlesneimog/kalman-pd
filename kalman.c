...
void kalman_setanalyze(t_kalman *x, t_float f) {
  if (f > 0) {
    ...
    post("kalman: analyze mode off.\nmean: %f, standard deviation: %f, noise "
         "covariance: %f",
...
void *kalman_new(t_symbol *s, int argc, t_atom *argv) {
  ...
  post("kalman filter initialized with %d iterations, %f noise covariance, and "
       "an init value of %f",
       x->iterations, x->noise_covariance, x->init_value);
...
void kalman_setup(void) {
  ...
  class_addmethod(kalman_class, (t_method)kalman_setinit, gensym("mean"),
                  A_FLOAT, 0);
  class_addmethod(kalman_class, (t_method)kalman_setanalyze, gensym("analyze"),
                  A_FLOAT, 0);
